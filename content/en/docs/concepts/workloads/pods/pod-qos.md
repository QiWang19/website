...
{{< caution >}}
With `TieredReservation`, Guaranteed pods have `memory.min` set equal to
`memory.max` (because requests equal limits). The kernel's `memory.min`
blocks **all** reclaim from that cgroup, including page cache generated by
the workload itself. This means file I/O–heavy Guaranteed pods cannot
reclaim their own page cache to make room for new allocations, making them
more likely to hit the OOM killer than they would without Memory QoS.

Burstable pods are not affected: `memory.low` is soft protection and does
not block intra-cgroup reclaim when the cgroup approaches its own
`memory.max`.

The default policy (`None`) sets `memory.min=0` for all pods and does not
change OOM behavior.
{{< /caution >}}
